(function () {
	...
	app.controller('updateCustomerController', updateCustomerController);

	updateCustomerController.$inject = ['$scope', 'datasource',
		'serverEndpoints', 'strings', 'navigation', 'authentication'];
	function updateCustomerController($scope, datasource, serverEndpoints,
		strings, navigation, authentication) {
		console.log("updateCustomerController()----> Start");
		var lvm = this;
		lvm.customers = [];
		lvm.messageTitle = "";
		lvm.message = "";
		...
		lvm.getCustomerDetailsById = function (adminId) {
			...
			datasource.getData(serverEndpoints.getCustomerByIdUrl,
				...
				$(".loader-background").css("display", "none");
					if (isSuccess) {
						...
						lvm.setOldVisas(data.customerVisas);
						lvm.setOldIntrest(data.customerInterests);
						lvm.setOldFrequentFlyers(data.customerFrequentFlyers);
					} else {/*
		...
		lvm.registerCustomer = function () {
			...
			lvm.WSurl = null;
			if (adminId == null || adminId === "") {
				lvm.WSurl = serverEndpoints.registerNewCustomerUrl;

			} else {
				registerCustomerData[serverEndpoints.id] = adminId;
				lvm.WSurl = serverEndpoints.updateCustomerUrl;
			}
			...
			registerCustomerData[serverEndpoints.customers]=customerObj;
			registerCustomerData[serverEndpoints.customerVisas] = visaDataArray;
			registerCustomerData[serverEndpoints.customerFrequentFlyers] = frequentDataArray;
			registerCustomerData[serverEndpoints.customerInterest] = lvm.interests;
			datasource
				.getData(
					lvm.WSurl,
		...
		console.log("updateCustomerController()---->End");
	}
